...
class ProjectAdmin(AuditedAdminMixin, TenantScopedAdmin, ExportCsvMixin, admin.ModelAdmin):
...
class MilestoneAdmin(AuditedAdminMixin, TenantScopedAdmin, ExportCsvMixin, admin.ModelAdmin):
...
@admin.register(Task)
class TaskAdmin(AuditedAdminMixin, TenantScopedAdmin, ExportCsvMixin, admin.ModelAdmin):
...
class DocumentAdmin(AuditedAdminMixin, TenantScopedAdmin, ExportCsvMixin, admin.ModelAdmin):
...
class TaskCommentAdmin(AuditedAdminMixin, TenantScopedAdmin, admin.ModelAdmin):
    list_display = ("task", "author", "created_at", "tenant")
    list_filter = ("tenant",)


@admin.register(ProjectChecklistItem)
class ProjectChecklistItemAdmin(AuditedAdminMixin, TenantScopedAdmin, admin.ModelAdmin):
    list_display = ("text", "project", "modality", "is_done", "tenant")
    list_filter = ("is_done", "modality", "tenant")


@admin.register(ProjectStakeholder)
class ProjectStakeholderAdmin(AuditedAdminMixin, TenantScopedAdmin, admin.ModelAdmin):
    list_display = ("project", "role", "display_name", "external_company", "tenant")
    list_filter = ("role", "tenant")


@admin.register(ProjectScopeAddition)
class ProjectScopeAdditionAdmin(AuditedAdminMixin, TenantScopedAdmin, admin.ModelAdmin):
